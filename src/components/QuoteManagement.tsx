                ...
                {filteredQuotes.map((quote) => (
                  <tr key={quote.id} className="hover:bg-gray-50 group">
                    <td className="px-4 py-4">
                      <div className="text-sm font-medium text-gray-900 truncate">{quote.customer.name}</div>
                      <div className="text-sm text-gray-500 truncate">{quote.customer.email}</div>
                      <div className="text-sm text-gray-500 whitespace-nowrap">{formatPhone(quote.customer.phone)}</div>
                    </td>
                    <td className="px-4 py-4 whitespace-nowrap">
                      <select
                        value={quote.status}
                        onChange={(e) => handleStatusChange(quote.id, e.target.value as Quote['status'])}
                        className={`max-w-full px-2 py-1 rounded-full text-xs font-medium border-0 focus:outline-none focus:ring-2 focus:ring-alencar-green ${getStatusColor(quote.status)}`}
                      >
                        <option value="new">Novo</option>
                        <option value="analyzing">Analisando</option>
                        <option value="negotiating">Negociando</option>
