...
export const BudgetSummaryContent: React.FC<BudgetSummaryContentProps> = ({
  ...
  return (
    <>
      <h3 className="text-xl font-bold text-[#b6ff28] mb-4">Simulação do seu projeto</h3>
      <div className="space-y-2 mb-4">
        <div className="flex justify-between">
          <span className="text-white/55">Container base:</span>
        ...
        {selectedItems.map((item) => (
          <div key={item.id} className="flex justify-between text-sm">
            ...
            <span className="font-semibold text-white/80">{formatCurrency(resolvePrice(item))}</span>
          </div>
        ))}
      </div>
